...
unless check_existing == "No AutoScalingGroups found"
  $stdout.puts("ERROR: snapshot is probably deployed already")
  $stdout.puts(check_existing)
  exit(1)
end

Commands.deploy_snapshot(snapshot, options[:min], options[:max], options[:capacity])
