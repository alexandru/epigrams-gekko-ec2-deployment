...
def provision():
    ...
    run("sudo aptitude -y install openjdk-7-jdk git typesafe-stack ruby1.9.3 supervisor htop iotop lsof sysstat")
    ...
    run("sudo mkdir -p /mnt/var/log/supervisor")
    run("sudo mkdir -p /mnt/var/tmp")
    run("sudo chmod ugo+rw /mnt/var/tmp")
